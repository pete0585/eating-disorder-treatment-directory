...
export default function SubmitForm() {
    ...
    if (submitted) {
        return (<div className="text-center py-16 px-6 max-w-md mx-auto">
    ...
    return (<form onSubmit={handleSubmit(onSubmit)} className="space-y-8 max-w-2xl mx-auto">
      {/* Type */}
      <div>
        ...
        ].map((opt) => (<label key={opt.value} className={`cursor-pointer border-2 rounded-xl p-4 transition-colors ${listingType === opt.value
                ? 'border-brand-plum bg-brand-plum-light'
                : 'border-gray-200 hover:border-brand-plum/50'}`}>
              <input type="radio" value={opt.value} {...register('listing_type')} className="hidden"/>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        {listingType === 'provider' ? (<div className="sm:col-span-2">
      ...
      <div>
        ...
        <textarea {...register('bio')} rows={4} className="form-input resize-none" placeholder="Brief description of your practice, specialties, and treatment approach..."/>
      </div>
      ...
      <div>
        ...
        <div className="flex flex-wrap gap-2">
          {DISORDERS.map((d) => (<button key={d.slug} type="button" onClick={() => toggleArray('disorders_treated', d.slug)} className={`text-sm px-3 py-1.5 rounded-full border transition-colors ${disordersTreated.includes(d.slug)
      ...
      <div>
        ...
        <div className="flex flex-wrap gap-2">
          {LEVELS_OF_CARE.map((l) => (<button key={l.slug} type="button" onClick={() => toggleArray('levels_of_care', l.slug)} className={`text-sm px-3 py-1.5 rounded-full border transition-colors ${levelsOfCare.includes(l.slug)
      ...
      <div>
        ...
        <div className="flex flex-wrap gap-2">
          {COMMUNITIES.map((c) => (<button key={c} type="button" onClick={() => toggleArray('communities_served', c)} className={`text-sm px-3 py-1.5 rounded-full border transition-colors ${communitiesServed.includes(c)
      ...
      <div className="space-y-3">
        <label className="flex items-center gap-3 cursor-pointer">
          <input type="checkbox" {...register('telehealth_available')} className="accent-brand-plum w-4 h-4"/>
          <span className="text-sm font-medium">Telehealth / Virtual sessions available</span>
        </label>
        <label className="flex items-center gap-3 cursor-pointer">
          <input type="checkbox" {...register('accepting_new_patients')} className="accent-brand-plum w-4 h-4"/>
      ...
      <button type="submit" disabled={isSubmitting} className="w-full btn-primary disabled:opacity-60 py-4 text-base">
